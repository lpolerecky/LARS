...
%
% Written by Lubos Polerecky, 2021-09-26, Utrecht University
...
% name of the input file (NOTE: assumed to be in the 'data' subfolder)
...
% create PNG subfolder where the visual output will be stored
if export_graphs_as_png && ~isfolder('png'), mkdir('png'); fprintf(1,'Subfolder png created.\n'); end

%% process input line by line
for ind=1:size(exp_data,1)
    
    %% extract data from the input table
    % note the order of the data columns
    t_incubation = exp_data(ind,1);
    xt = exp_data(ind,2); % xt corresponds to the 13C atom fraction of the cell        
    dxt = exp_data(ind,3);
    xS = exp_data(ind,4);
    xini = exp_data(ind,5);
    rhoC = exp_data(ind,6);
    ...
    %% if both C and <C> of the measured cell can be constrained
    if ~isnan(avgCcell) && ~isnan(Ccell)
        ...
        %% if no errors for x and C are given, only the mean values of the rates can be estimated            
        if dxt<3*eps && dCcell<3*eps
            ...
            %% reconstruct the cell cycle (St) and number of cell divisions during SIP incubation
            % zero-order kinetics
            ...
            %% Use a Monte-Carlo approach if errors of x or C are given
            
            %% generate randomly distributed values of x and s_end
            % x is assumed to be *normally* distributed (mean=xt, SD=dxt)
            ...
            if abs(ds_end)<3*eps
                ...
                % if dCcell is positive, treat the cell as a representative
                % from a population of cells with partially synchronized
                % cell cycles (see Eq. 17 in the manuscript how partially
                % synchronized cells are modelled)
            ...
            %% test values, only used during debugging
            ...
            for j=1:Nsimul2
                ...
                % choose a specific pair [x,C] from the randomly
                % distributed values derived from the experimental data
                xSEtj = xSEt_rnd(j);
                C_endj = Crand(j);
                ...
                if xSEtj>0 && xSEtj<1
                    ...
                    % this should not happen, so issue a warning
            ...
            %% end of the Monte-Carlo approach
        ...
        %% if C or <C> cannot be constrained for the measured cell, we do 
        % not use the Monte-Carlo approach but simply calculate mean
        % values and errors using the formulas and the rules for error
        % propagation. This will optimize speed.
        ...
        % if only <C> is available, estimate r_A
        if ~isnan(avgCcell)
            rC_A      = kC * avgCcell;
        ...
        % if only C is available, estimate r_B and r_nondiv
        if ~isnan(Ccell)
            rC_A    = NaN;
            drC_A   = NaN;
            rC_B    = kC * Ccell;
        ...
        % if neither C nor <C> is available, only kC can be estimated (calculated above)
...
%% convert output to a nice table
...
function dx_val = dx(rC, x_end, C_end, t_end, avgCcell)
    % calculate the difference between the predicted and measured x; zero of
    % this function is sought to find the solution r = Z^{-1}(...) (Approach C)
...
function [Ct, x13Ct, t, tau, St] = x13C_time0(rate, C_end, avgCcell, t_incubation, xSEt)
    % model for predicting 13C atom fraction in a *linearly* growing cell (the
    % "zig-zag" function, Z)
...
% 18 (see also Fig. S3) in Polerecky et al. (submitted to FrontMic).
    N0=2000;
    m_edge=linspace(Cmax/2,Cmax,N0+1)';
    dm=diff(m_edge);
    m_mid=m_edge(1:N0)+dm(1)/2;
...
%% plot results for the current cell
    ...
    xval=xval(:);
    frand=figure;
    h1=histogram2(xval,St100_0(:),'DisplayStyle','tile','ShowEmptyBins','on',...
        'XBinEdges',linspace(min(t100),max(t100),101),'YBinEdges',linspace(0,1,101),...
        'Normalization','pdf');
    d1=h1.Values;
    h2=histogram2(xval,x13Ct100_0(:),'DisplayStyle','tile','ShowEmptyBins','on',...
        'XBinEdges',linspace(min(t100),max(t100),101),'YBinEdges',linspace(0,1,101),...
        'Normalization','pdf');
    d2=h2.Values;
    ...
    thr=0.01;
    r=rgb(:,:,1);
    r(d2>thr)=rescale(log10(d2(d2>thr))); g=r; b=r; 
    r(d1>thr)=rescale(log10(d1(d1>thr)));
    ...
    xlabel('x_j')
    ylabel('C_j')
    tit=sprintf('Experimental values (C-nondiv = %.2f +/- %.2f)', ...
        mean(Crand_nondiv), std(Crand_nondiv));
    title(tit)

    s3=subplot(2,3,3);
    histogram2(xt_rnd,Crand,'DisplayStyle','tile','ShowEmptyBins','on',...
        'EdgeAlpha',0,'NumBins',[30 30],'Normalization','pdf');
    xlabel('x_j')
    ylabel('C_j')
    ...
    s4=subplot(2,3,4);
    hold off;
    histogram(s0_ini_rnd,30,'EdgeColor',[1 0 0],'FaceColor',0.9*[1 1 1]);
    hold on;
    Cmax = avgCcell*2*log(2);
    histogram(Crand/(Cmax/2)-1,30,'EdgeColor',[0 0 1],'FaceColor',0.9*[1 1 1]);
    xlabel('s');
    ylabel('count');
    ...
    s5=subplot(2,3,5);
    histogram2(rC_C,s0_ini_rnd,'DisplayStyle','tile','ShowEmptyBins','on',...
        'EdgeAlpha',0,'NumBins',[30 30],'Normalization','pdf');
